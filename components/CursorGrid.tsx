...
export default function CursorGrid({
  ...
  useEffect(() => {
    ...
    const rebuild = () => {
      ...
      w = container.offsetWidth || container.parentElement?.offsetWidth || window.innerWidth;
      h = container.offsetHeight || container.parentElement?.offsetHeight || 600;
    ...
    // Window-level listener with canvas bounding rect checking
    const onPointerMove = (e: PointerEvent) => {
      const rect = canvas.getBoundingClientRect();
      const x = e.clientX - rect.left;
      const y = e.clientY - rect.top;
      const r = (propsRef.current.radius || 120) + 50;

      // Only wake/energize if cursor is over or near the canvas
      if (
        x >= -r &&
        x <= rect.width + r &&
        y >= -r &&
    ...
    // Listen on window so events are never blocked by z-index or child overlays
